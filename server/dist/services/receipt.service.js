...
exports.generate80GReceipt = void 0;
 ...
 */
const generate80GReceipt = async (auctionId, res) => {
    ...
    const ngo = await ngo_model_1.Ngo.findById(auction.ngo);
    if (!ngo)
    ...
    const winner = await user_model_1.User.findById(settlement.winner);
    ...
    res.setHeader('Content-Disposition', `attachment; filename=80G_Receipt_${auctionId}_${Date.now()}.pdf`);
