...
import { Settlement, ISettlementTransfer } from '../models/settlement.model';

const Razorpay = require('razorpay');

export interface SettlementInput {
  auctionId: string;
  finalPaymentId: string; // Razorpay pay_xxxx from the winner's final payment
  settledBy: string;      // Admin userId triggering the settlement
}
...
export const executeSettlement = async (
  ...
  if (!winner.taxProfile?.isComplete) {
    return {
      success: false,
      message:
        'Cannot settle: winner\'s Tax Profile (PAN, address) is incomplete. Required for 80G compliance.',
    };
  }

  // ── 3. Idempotency Guard ─────────────────────────────────────────────────
  const existingSettlement = await Settlement.findOne({ auctionItem: new Types.ObjectId(auctionId) });
  ...
  const transferPromises = auction.payoutConfigs.map(async (config) => {
    ...
    const transferRecord: ISettlementTransfer = {
    ...
    try {
      ...
      console.log(
        `  ✅ Transfer [${config.recipient}] ₹${legAmount} → ${config.accountId} | ID: ${transfer.id}`
      );
    } catch (err: any) {
      transferRecord.failureReason = err?.error?.description || err?.message || 'Unknown Razorpay error';
      console.error(
        `  ❌ Transfer [${config.recipient}] FAILED → ${config.accountId} | Reason: ${transferRecord.failureReason}`
      );
    }
  ...
  const overallStatus: 'COMPLETED' | 'PARTIAL_FAILURE' = allSucceeded
    ? 'COMPLETED'
    : 'PARTIAL_FAILURE';
  ...
  const settlementDoc = await Settlement.create({
    auctionItem: new Types.ObjectId(auctionId),
  ...
  if (allSucceeded) {
    ...
    console.warn(
      `⚠️  Auction [${auctionId}] settlement PARTIAL_FAILURE — requires manual review.`
    );
  }
